...
class ReservationControl extends Controller
{
    public function data(Request $req)
    {
      ...
      $data = Reservation::join('jadwal','jadwal.id','=', 'reservation.id_jadwal')
            ->join('users','users.id','=','reservation.userid')
            ->join('customer','customer.id','=','reservation.customerid')
            ->join('rute','rute.id','=','jadwal.id_rute')
    ...
    {
      return view('pages.reservation.pesan',['id_customer'=>$req->id_customer,
            'id_type'=>$req->id_type]);
    }

    public function batal(Request $req) {
        $result = Reservation::where('id',$req->id)
            ->update([
                'keterangan'=>$req->ket,
            ]);

        if($result){
            return redirect()->route('reservation.data')->with('status-alert','sukses-dibatal');
        } else{
  ...
  public function simpan(Request $req)
    {
       $valid = Validator::make($req->all(),[
                ...
                'tempat_duduk'=>'required|between:2,50|unique:reservation,seat_code,null,id,id_jadwal,'.$req->jadwal.',depart_at,'.$req->tanggal,
