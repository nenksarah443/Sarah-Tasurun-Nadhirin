...
class PenggunaControl extends Controller
{
    ...
    public function simpan(Request $req)
    {
    	$valid = Validator::make($req->all(),[
    		'nama'=>'required|between:2,50',
    		'email'=>'required|between:8,50|email|unique:users,email',
            'username'=>'required|between:2,50|unique:users,username|alpha_dash',
    ...
    public function update(Request $req)
    {
        $valid = Validator::make($req->all(),[
            ...
            'username'=>'required|between:2,50|unique:users,username,'.$req->id.'|alpha_dash',
            'level'=>'required|in:admin,operator',
   ...
   {
      return view('pages.pengguna.buat');
   }
   ...
   public function render(Request $req)
   {
       $valid = Validator::make($req->all(),[
        ...
        if($valid->fails()){
            return redirect()->route('pengguna.report')
                    ->withErrors($valid)
                    ->withInput()
                    ->with('status-alert','peringatan');
         }
         ...
         $data = User::select(
               ...
               ->where('level', 'like', "%$req->level%") 
               ->OrderBy('users.id', 'asc')
               ->get();
         return view('pages.pengguna.render',['data'=>$data, 'level'=>$req->level]);
