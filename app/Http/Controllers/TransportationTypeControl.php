...
class TransportationTypeControl extends Controller
{
   ...
   {
      return view('pages.transportation-type.buat');
   }
   ...
   public function render(Request $req)
   {
       $valid = Validator::make($req->all(),[
        ...
        if($valid->fails()){
            return redirect()->route('transportation-type.report')
                    ->withErrors($valid)
                    ->withInput()
                    ->with('status-alert','peringatan');
         }

         $data = TransportationType::select(
                  'transportation_type.*'
                  )
               
               ->where('description', 'like', "%$req->description%") 
               ->OrderBy('transportation_type.id', 'asc')
               ->get();
         return view('pages.transportation-type.render',['data'=>$data, 'description'=>$req->description]);
