...
class CustomerControl extends Controller
{
 ...
 public function buat(Request $req)
   {
      return view('pages.customer.buat');
   }
   ...
   public function render(Request $req)
   {
       $valid = Validator::make($req->all(),[
        ...
        if($valid->fails()){
            return redirect()->route('customer.report')
                    ->withErrors($valid)
                    ->withInput()
                    ->with('status-alert','peringatan');
         }
         ...
         $data = Customer::select(
               ...
               ->where('address', 'like', "%$req->alamat%") 
               ->get();
         return view('pages.customer.render',['data'=>$data, 'address'=>$req->alamat]);
